...
def feature_scale(
    data: np.ndarray,
    scaler: scalers.Scaler | Type[scalers.Scaler],
    max_length: int = None,
) -> Tuple[scalers.Scaler, torch.Tensor]:
...
def pad_tensors(
    numpy_list: np.ndarray, padding_value: Any = np.nan, max_length: int = None
) -> torch.Tensor:
...
def fit_scalers(
    ...
    features_scaling = [
        ...
        np.array(list(map(lambda x: x[0], salinity)))[
            :, np.newaxis
        ],  # Surface salinity
        np.array(list(map(lambda x: x[0], temperatur)))[
            :, np.newaxis
        ],  # Surface temperatur
        np.array(list(map(lambda x: x[0], dataset.depth)))[
            :, np.newaxis
        ],  # "Surface" depth
        get_bathymetri(
            dataset.lat,
...
def to_dataloader(
    features: torch.Tensor, labels: torch.Tensor, batch_size: int = 16
) -> DataLoader:
    ...
    if torch.cuda.is_available():
        ...
        return DataLoader(
            TensorDataset(features.squeeze(0), labels.squeeze(0)),
            batch_size=batch_size,
            shuffle=False,
        )

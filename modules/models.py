...
class TSNet(torch.nn.Module):
    def __init__(
        ...
        linear_settings: List[torch_util.Dict],
        model_type: str,
        dropout: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.fc = torch.nn.Sequential(
            *[
                torch.nn.Linear(ls.input_size, ls.output_size).double()
                for ls in self.linear_settings
            ]
        )
    ...
    def train_epoch(
        self, epoch: int, dataloader: DataLoader, optimizer, scheduler, criterion
    ) -> Tuple[float, float]:
...
def load_model(train_loader: DataLoader) -> TSNet:
    ...
    rnn_setting = torch_util.Dict(
        input_size=features.size(1), output_size=labels.size(1), n_layers=cfg.gru_layers
    )
    ...
    linear_settings = [
        torch_util.Dict(input_size=d1, output_size=d2)
...
def train_model(
...
) -> Tuple[List[TrainingLoss], List[ValidationLoss]]:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=cfg.T_0, T_mult=cfg.T_mult, eta_min=0.001, last_epoch=-1
    )
    ...
    for i in range(cfg.epochs):
        # Training
        train_loss, lr = model.train_epoch(
            i, train_loader, optimizer, scheduler, criterion
        )
